...
impl AggregateStatus {
    ...
    #[must_use]
    pub const fn badge_bg_color(self) -> &'static str {
        match self {
            Self::Pending => "#94a3b8",
            Self::Running => "#2563eb",
...
fn calculate_bounding_box(branch_nodes: &[Node]) -> Option<BoundingBox> {
    ...
    Some(BoundingBox {
        x: min_x - PADDING_X,
        y: min_y - PADDING_Y,
        width: (max_x - min_x).mul_add(1.0, PADDING_X * 2.0),
        height: (max_y - min_y).mul_add(1.0, PADDING_Y * 2.0),
    })
}
...
mod tests {
    ...
    fn make_node(id: Uuid, node_type: &str, x: f32, y: f32) -> Node {
        Node {
            ...
            category: crate::graph::NodeCategory::Flow,
