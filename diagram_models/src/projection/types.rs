...
use crate::document::{DiagramDocument, DocumentData, Edge, EdgeId, Node, NodeId};
use crate::envelope::{Author, DomainOp};
...
pub fn projection_to_document(projection: &DiagramProjection) -> DiagramDocument {
    DiagramDocument {
        ...
        revision: crate::document::Revision::new(projection.revision),
        document: DocumentData {
            nodes: projection.nodes.clone(),
            edges: projection.edges.clone(),
        },
        editor_state: crate::document::EditorState::default(),
    }
}
