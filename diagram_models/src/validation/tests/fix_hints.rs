...
fn all_known_codes_have_hints() {
    assert_hint!(
        ...
        "Remove edge {edge_id} or create missing node {missing_node_id}"
    );
    assert_hint!(
        ValidationCode::INVALID_PARENT,
        "Set node {node_id}.parent to None or reference an existing Subgraph node"
    );
    assert_hint!(
        ValidationCode::INVALID_NUMERIC,
        "Ensure numeric field {field_name} is finite. Got: {actual_value}"
    );
    assert_hint!(
        ValidationCode::DAG_CYCLE,
        "Remove one edge in the cycle to break it. Cycle path: {cycle_path}"
    );
    assert_hint!(
        ValidationCode::DAG_DISCONNECTED,
        "Add edges to connect all {n} components into a single connected graph"
    );
    assert_hint!(
        ValidationCode::INTERNAL_ERROR,
        "This is a bug. Report to developers with reproduction steps."
    );
    assert_hint!(
        ValidationCode::SCHEMA,
        "Fix schema violation at {path}: expected {expected}, got {actual}"
    );
    assert_hint!(
        ValidationCode::INVALID_VERSION,
        "Set document version to 2. Current: {actual}"
    );
    assert_hint!(
        ValidationCode::PARENT_CYCLE,
        "Break the parent chain cycle by setting {node_id}.parent = None"
    );
    assert_hint!(
        ValidationCode::EDGE_INVALID_OFFSET,
        "Set edge {edge_id}.label_offset_t to a value in [0.0, 1.0]. Got: {actual}"
    );
    assert_hint!(
        ValidationCode::EDGE_INVALID_THICKNESS,
        "Set edge {edge_id}.thickness to a finite non-negative value. Got: {actual}"
    );
    assert_hint!(
        ValidationCode::EDGE_INVALID_COLOR,
        "Set edge {edge_id}.color to hex format #RGB, #RGBA, #RRGGBB, or #RRGGBBAA. Got: {actual}"
    );
    assert_hint!(
        ValidationCode::EDGE_INVALID_FONT_SIZE,
        "Set edge {edge_id}.font_size to a finite value. Got: {actual}"
    );
    assert_hint!(
        ValidationCode::EDITOR_INVALID_STATE,
        "Set editor.{field} to a finite value. Got: {actual}"
    );
}
...
fn error_populates_fix_hint() {
    ...
    assert_eq!(
        issue.fix_hint,
        Some("Remove edge {edge_id} or create missing node {missing_node_id}".to_string())
    );
...
fn with_fix_hint_sets_custom() {
    ...
    assert_eq!(
        issue.fix_hint,
        Some("Custom fix: delete the edge".to_string())
    );
}
