...
pub fn set_node_parent_ext(
    ...
    if keep_world_pos {
        ...
        let rel_x = crate::document::OrderedFloat::new(world_x - parent_world_x).map_err(|_| {
            Error::InvalidTransform // Or a more appropriate error
        })?;
        let rel_y = crate::document::OrderedFloat::new(world_y - parent_world_y)
...
pub fn unparent_node_ext(
    ...
    if keep_world_pos {
        ...
        node.parent = None;
        node.x =
            crate::document::OrderedFloat::new(world_x).map_err(|_| Error::InvalidTransform)?;
        node.y =
        ...
        let updated_node = canvas
            ...
            .get(&child_id)
            .cloned()
            .map(|n| crate::document::Node { parent: None, ..n })
            .ok_or_else(|| Error::NodeNotFound(child_id.clone()))?;
