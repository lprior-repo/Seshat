...
async function waitForZoomStable(page: Page, expected: number, tolerance = 5): Promise<boolean> {
...
test.describe("CAM viewport and zoom behavior @baseline", () => {
  test.describe("wheel zoom anchor", () => {
    test("wheel zoom at cursor keeps node centered under pointer @p0-smoke", async ({ page }) => {
    ...
    test("wheel zoom out preserves anchor point", async ({ page }) => {
      const pageErrors = trapPageErrors(page);
      await freshStart(page);
      await clearCanvasOverlays(page);
      ...
      const after = await runEffect(() => node.boundingBox());
      if (!after) {
        throw new Error("node bounds unavailable after zoom out");
      }

      const afterCenterX = after.x + after.width / 2;
      const afterCenterY = after.y + after.height / 2;
      expect(Math.abs(afterCenterX - anchorX)).toBeLessThan(24);
      expect(Math.abs(afterCenterY - anchorY)).toBeLessThan(24);
      expect(pageErrors).toHaveLength(0);
    });
  });

  test.describe("spacebar pan", () => {
    test("spacebar + drag pans viewport without selecting nodes", async ({ page }) => {
      const pageErrors = trapPageErrors(page);
      await freshStart(page);
      await clearCanvasOverlays(page);
    ...
    test("spacebar pan then release spacebar returns to select mode", async ({ page }) => {
      const pageErrors = trapPageErrors(page);
      await freshStart(page);
      await clearCanvasOverlays(page);
      ...
      await runEffectsSequential([
        () => page.keyboard.down("Space"),
        () => page.mouse.move(cbox.x + 200, cbox.y + 150),
        () => page.mouse.down(),
        () => page.mouse.move(cbox.x + 350, cbox.y + 250, { steps: 4 }),
        () => page.mouse.up(),
        () => page.keyboard.up("Space"),
      ]);

      // Now clicking should work normally (select mode restored)
      await runEffect(() => createTextNode(page, canvas, 600, 350));
  ...
  test.describe("zoom clamping", () => {
    test("zoom out clamps at minimum 10%", async ({ page }) => {
      const pageErrors = trapPageErrors(page);
      await freshStart(page);
      await clearCanvasOverlays(page);
    ...
    test("zoom in clamps at maximum 400%", async ({ page }) => {
      const pageErrors = trapPageErrors(page);
      await freshStart(page);
      await clearCanvasOverlays(page);
    ...
    test("wheel zoom cannot exceed maximum even with large delta", async ({ page }) => {
      const pageErrors = trapPageErrors(page);
      await freshStart(page);
      await clearCanvasOverlays(page);
      ...
      // First zoom to near max
      await zoomInUntil(page, 380);

      // Try extreme wheel zoom
      await runEffectsSequential([
  ...
  test.describe("world-to-screen coordinate transform", () => {
    test("world-to-screen remains consistent at extreme zoom levels", async ({ page }) => {
      const pageErrors = trapPageErrors(page);
      await freshStart(page);
      await clearCanvasOverlays(page);
    ...
    test("coordinate transform handles negative world coordinates", async ({ page }) => {
      const pageErrors = trapPageErrors(page);
      await freshStart(page);
      await clearCanvasOverlays(page);

      // Pan camera to negative world space
      const canvas = page.getByTestId("canvas-root");
      const cbox = await canvasBox(page);
      ...
      await runEffectsSequential([
        ...
        () => page.mouse.move(cbox.x + cbox.width / 2, cbox.y + cbox.height / 2),
        () => page.mouse.down(),
        () => page.mouse.move(cbox.x + cbox.width / 2 + 400, cbox.y + cbox.height / 2 + 300, { steps: 8 }),
        () => page.mouse.up(),
        () => page.keyboard.up("Space"),
      ]);

      // Create node in what is now negative world space
      await runEffect(() => createTextNode(page, canvas, 200, 150));
      expect(await nodeCount(page)).toBe(1);
  ...
  test.describe("scrollable parent embedding", () => {
    test("wheel zoom works when canvas is in scrollable container", async ({ page }) => {
      const pageErrors = trapPageErrors(page);
      await freshStart(page);
      await clearCanvasOverlays(page);
    ...
    test("pan gesture works within scrollable container", async ({ page }) => {
      const pageErrors = trapPageErrors(page);
      await freshStart(page);
      await clearCanvasOverlays(page);
      await mountScrollableHarness(page);
      await scrollHarnessTo(page, 150, 250);

      const canvas = page.getByTestId("canvas-root");
      await runEffect(() => createTextNode(page, canvas, 350, 250));
      ...
      await runEffectsSequential([
        ...
        () => page.mouse.move(startX, startY),
        () => page.mouse.down(),
        () => page.mouse.move(startX + 120, startY + 80, { steps: 6 }),
        () => page.mouse.up(),
        () => page.keyboard.up("Space"),
      ]);
  ...
  test.describe("edge scrolling during drag", () => {
    test("drag near scroll parent edge updates scroll position", async ({ page }) => {
      const pageErrors = trapPageErrors(page);
      await freshStart(page);
      await clearCanvasOverlays(page);
      await mountScrollableHarness(page);
  ...
  test.describe("device pixel ratio changes", () => {
    test("viewport recalculates after DPR simulation", async ({ page }) => {
      const pageErrors = trapPageErrors(page);
      await freshStart(page);
      await clearCanvasOverlays(page);
      ...
      // Simulate DPR change by resizing viewport (affects layout)
