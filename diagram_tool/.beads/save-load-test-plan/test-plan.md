...
**Then**: Returns `Err(OpenError::Validation(s))` where `s.contains("version")`

**Test name**: `fn apply_open_document_returns_validation_error_for_missing_version()`
...
**Then**: `doc` still has 3 nodes (unchanged), `history` still has prior state (unchanged)
...
| Parse error: missing version | JSON without version field | Err(OpenError::Validation(contains "version")) | unit |
...
| Variant | Covered By | Test Name |
|---------|------------|-----------|
| Parse(String) | Behavior 30 | `apply_open_document_returns_parse_error_for_invalid_json` |
| Validation(String) | Behavior 31 | `apply_open_document_returns_validation_error_for_missing_version` |
| Io(String) | Behavior 25 | `open_workspace_shows_error_toast_on_native_io_failure` |
