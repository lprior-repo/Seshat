...
pub fn calculate_grid_layout(doc: &DiagramDocument, cell_size: f64) -> DiagramDocument {
    let occupied_cells = doc.document.nodes.values()
        .filter(|node| node.locked && node.parent.is_none())
        .map(|node| ((node.x.0 / cell_size).round() as i32, (node.y.0 / cell_size).round() as i32))
        .collect::<im::HashSet<(i32, i32)>>();

    let unlocked_ids = doc.document.nodes.iter()
        .filter(|(_, n)| !n.locked && n.parent.is_none())
        .map(|(id, _)| id.clone())
        .sorted()
        .collect::<Vec<NodeId>>();

    if unlocked_ids.is_empty() { return doc.clone(); }

    #[allow(clippy::cast_precision_loss)]
    let cols_target = (unlocked_ids.len() as f64).sqrt().ceil() as i32;

    let (_, _, _, positions) = unlocked_ids.iter().fold((occupied_cells, 0_i32, 0_i32, HashMap::new()), |(mut occupied, mut col, mut row, pos_map), id| {
        let (new_col, new_row) = (0..).find_map(|_| {
        ...
        let new_pos = (f64::from(new_col) * cell_size, f64::from(new_row) * cell_size);
    ...
    let deltas: HashMap<NodeId, (f64, f64)> = positions.iter().filter_map(|(id, (nx, ny))| {
        doc.document.nodes.get(id).map(|node| (id.clone(), (nx - node.x.0, ny - node.y.0)))
    }).collect();

    let next_nodes = doc.document.nodes.iter().map(|(id, node)| {
        match positions.get(id) {
            ...
            None => {
                node.parent.as_ref().map_or_else(|| (id.clone(), node.clone()), |pid| {
                    deltas.get(pid).map_or_else(|| (id.clone(), node.clone()), |&(dx, dy)| {
                        let mut next_node = node.clone();
