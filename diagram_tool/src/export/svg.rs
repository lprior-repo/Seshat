...
pub fn generate_svg_string(doc: &DiagramDocument) -> String {
    ...
    let _ = write!(&mut svg, "<svg xmlns='http://www.w3.org/2000/svg' viewBox='{view_min_x} {view_min_y} {width} {height}' width='{width}' height='{height}'>");
    
    // Edges
    doc.document.edges.values().for_each(|edge| {
        if let Some((src, tgt)) = doc.document.nodes.get(&edge.source).zip(doc.document.nodes.get(&edge.target)) {
            ...
            let _ = write!(&mut svg, "<line x1='{sx}' y1='{sy}' x2='{tx}' y2='{ty}' stroke='black' stroke-width='2' />");
        }
    });
    ...
    doc.document.nodes.values().for_each(|node| {
         ...
         let _ = write!(&mut svg, "<rect width='{}' height='{}' fill='white' stroke='black' rx='4' ry='4'/>", node.width.0, node.height.0);
         ...
         if let Some(file) = ICONS.get_file(&node.icon) {
             ...
             let _ = write!(&mut svg, "<image href='data:image/png;base64,{b64}' width='{icon_size}' height='{icon_size}' x='{ix}' y='{iy}' />");
         }
         
         let _ = write!(&mut svg, "<text x='{}' y='{}' text-anchor='middle' font-family='sans-serif' font-size='10'>{}</text>", node.width.0 / 2.0, node.height.0 - 5.0, node.label);
...
fn calculate_bounds(doc: &DiagramDocument) -> (f64, f64, f64, f64) {
    if doc.document.nodes.is_empty() {
        ...
        doc.document.nodes.values().fold((f64::MAX, f64::MAX, f64::MIN, f64::MIN), |(min_x, min_y, max_x, max_y), node| {
            (
                min_x.min(node.x.0),
                min_y.min(node.y.0),
