...
mod tests {
    ...
    fn given_preserve_policy_when_run_mutation_then_revision_is_not_incremented() {
        ...
        let result = run_mutation_with_policy(
            ...
            crate::mutation::pipeline::ValidationPolicy::default(),
            |doc| Ok(doc.clone()),
        );
...
mod proptests {
    ...
    proptest! {
            ...
            let result = run_mutation(&current, move |_| {
                Ok(DiagramDocument {
                    editor_state: EditorState {
                        ...
                        theme: diagram_models::document::EditorTheme::System,
                        show_grid: true,
        ...
        fn locked_node_modification(_attempt_modify in any::<bool>()) {
            ...
            prop_assert!(node.lock_state.is_locked());
        }
