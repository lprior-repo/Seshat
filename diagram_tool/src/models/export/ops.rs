...
use crate::store::{self, ReadOnlySession, ReadWriteSession, ValidEvent};
use serde::{Deserialize, Serialize};

use super::{
    Author, DiagramJsonExport, DiagramMetadata, EnvelopeAuthor, ExportError, ImportResult,
    DEFAULT_DIAGRAM_NAME, EXPORT_SCHEMA_VERSION,
};

const SUPPORTED_VERSION: u32 = 2;

pub fn export_diagram_json(session: &ReadOnlySession) -> Result<DiagramJsonExport, ExportError> {
    let events = fetch_all_events(session)?;
...
pub fn export_while_recovering(session: &ReadOnlySession) -> Result<String, ExportError> {
    let events = fetch_all_events(session)?;
    let projection = replay_events_from_db(&events)?;
...
pub fn fetch_all_events(session: &ReadOnlySession) -> Result<Vec<EventRecord>, ExportError> {
    let raw_events =
        store::fetch_all_events(session).map_err(|e| ExportError::Sqlite(e.to_string()))?;
    let mut events = Vec::with_capacity(raw_events.len());
    for raw in raw_events {
        if let Ok(envelope) = parse_event_envelope(&raw.payload) {
...
#[must_use]
fn projection_to_document(
    projection: &DiagramProjection,
) -> crate::models::document::DiagramDocument {
...
pub fn import_diagram_json(
    ...
    let events_to_import = export
        .events
        .unwrap_or_else(|| generate_canonical_events(&projection));
    ...
    for event_record in &event_records {
        ...
        let valid_event = ValidEvent::new(envelope);
        match store::append_idempotent(session, valid_event) {
            Ok(_) => events_imported += 1,
            Err(e) => {
                let err_str = e.to_string();
                if err_str.contains("UNIQUE constraint failed")
                    || err_str.contains("constraint failed")
                {
                    continue;
                }
                return Err(ExportError::Sqlite(err_str));
            }
        }
    }
    Ok(ImportResult {
        events_generated: events_imported,
        final_revision: event_records.len() as u64,
    })
}
...
fn generate_canonical_events(projection: &DiagramProjection) -> Vec<serde_json::Value> {
    ...
    for (id, edge) in projection.edges.iter() {
        let envelope = EventEnvelope {
            op_id: format!("gen-edge-{}", id),
            operation: crate::models::envelope::DomainOp::EdgeConnect {
                id: id.to_string(),
