...
use im::HashMap;
use crate::models::document::{Node, Edge, NodeId, EdgeId};
use thiserror::Error;
use tap::Tap;
...
pub fn validate_dag(nodes: &HashMap<NodeId, Node>, edges: &HashMap<EdgeId, Edge>) -> Result<(), CycleError> {
    let in_degree_init = nodes.keys()
        .map(|id| (id.clone(), 0))
        .collect::<HashMap<NodeId, usize>>();

    let (adjacency, in_degree) = edges.values()
        .filter(|e| nodes.contains_key(&e.source) && nodes.contains_key(&e.target))
        .fold((HashMap::<NodeId, Vec<NodeId>>::new(), in_degree_init), 
            |(adj, deg), edge| {
                (
                    adj.get(&edge.source).map_or_else(
                        || adj.update(edge.source.clone(), vec![edge.target.clone()]),
                        |neighbors| adj.update(edge.source.clone(), neighbors.clone().tap_mut(|n| n.push(edge.target.clone())))
                    ),
    ...
    let initial_queue = in_degree.iter()
        .filter(|&(_, &deg)| deg == 0)
        .map(|(id, _)| id.clone())
        .collect::<VecDeque<NodeId>>();

    let final_state = (0..nodes.len()).fold((initial_queue, in_degree, 0), |(mut q, degs, count), _| {
        match q.pop_front() {
            Some(node_id) => {
                let neighbors = adjacency.get(&node_id).map_or_else(Vec::new, Clone::clone);
                let (next_q, next_degs) = neighbors.into_iter().fold((q, degs), |(mut cq, cd), neighbor| {
                    let next_count = cd.get(&neighbor).map_or(0, |&c| if c > 0 { c - 1 } else { 0 });
                    if next_count == 0 { cq.push_back(neighbor.clone()); }
    ...
    if final_state.2 == nodes.len() {
        ...
        Err(
            match edges.iter().find(|(_, edge)| {
                match (final_state.1.get(&edge.target), final_state.1.get(&edge.source)) {
