...
pub fn apply_node_add(
    ...
    let mut new_nodes = state
        .nodes
        .update(node_id, create_default_node(x, y, width, height, label));

    // GEO-026: Propagate bounds changes to ancestor containers
    new_nodes = propagate_bounds_to_ancestors(new_nodes, &node_id_for_propagation);
...
pub fn apply_node_move(
    ...
    let mut new_nodes = state.nodes.update(node_id, updated_node);
...
pub fn apply_node_delete(
    ...
    // Get parent containers BEFORE deleting the node
