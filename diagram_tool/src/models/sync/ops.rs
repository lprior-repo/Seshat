...
use crate::store;
use crate::store::ReadOnlySession;
                ...
                if matches!(event.kind, EventKind::Modify(_)) {
                    let _is_db_change = event.paths.iter().any(|p| {
                        p.to_string_lossy().ends_with(".db")
                            || p.to_string_lossy().ends_with("-wal")
                    });
                }
            }
        },
...
pub fn fetch_new_events(
    ...
    let raw_events = store::fetch_events_since(session, after_revision)
    ...
    for raw in raw_events {
        if raw.revision != expected_revision {
            return Err(SyncError::Decode("revision gap".to_string()));
        }
        let envelope = parse_event_envelope(&raw.payload)
            .map_err(|_| SyncError::Decode("envelope parse error".to_string()))?;
...
#[cfg(not(target_arch = "wasm32"))]
pub fn fetch_latest_revision(session: &ReadOnlySession) -> Result<i64, SyncError> {
    store::fetch_latest_revision(session).map_err(|e| SyncError::Sqlite(e.to_string()))
}
...
pub fn apply_tail_batch(
    ...
    let affected_entities: Vec<String> = events
        .iter()
        .flat_map(|e| match &e.operation {
            crate::models::envelope::DomainOp::NodeAdd { id, .. }
            | crate::models::envelope::DomainOp::NodeMove { id, .. }
            | crate::models::envelope::DomainOp::NodeDelete { id }
            | crate::models::envelope::DomainOp::NodeRestore { id } => vec![format!("node:{}", id)],
            _ => vec![],
        })
        .collect();
...
pub fn schedule_ui_update(_summary: ApplySummary) -> Result<(), SyncError> {
    Ok(())
}
