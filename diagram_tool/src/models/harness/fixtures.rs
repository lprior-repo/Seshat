...
pub fn run_replay_fuzz(seed: u64, cases: usize) -> Result<FuzzReport, VerifyError> {
    ...
    for _ in 0..cases {
        ...
        let event = match op_type {
            0 => {
                ...
                EventRecord {
                    op_id: format!("fuzz-{}", rng.next()),
                    revision,
                    operation: DomainOp::NodeAdd {
                        id: node_id,
                        x: (rng.next() % 1000) as f64,
                        y: (rng.next() % 1000) as f64,
                        width: 100.0,
                        height: 50.0,
                        label: format!("Node {}", node_counter),
                    },
                    author: Author {
                        id: format!("author-{}", author_counter),
                        name: format!("Author {}", author_counter),
                        email: None,
                    },
            ...
            1 if node_counter > 0 => {
                let node_id = format!("node-{}", (rng.next() % node_counter) + 1);
                EventRecord {
                    op_id: format!("fuzz-{}", rng.next()),
                    revision,
            ...
            2 if node_counter > 1 => {
                let source = format!("node-{}", (rng.next() % node_counter) + 1);
                let target = format!("node-{}", (rng.next() % node_counter) + 1);
                if source != target {
                    EventRecord {
                        op_id: format!("fuzz-{}", rng.next()),
                        revision,
                        operation: DomainOp::EdgeConnect {
                            id: format!("edge-{}", rng.next()),
                            source,
                            target,
                        },
                        author: Author {
    ...
    let projection =
        replay_events(&all_events).map_err(|e| VerifyError::TestHarness(e.to_string()))?;
...
pub fn assert_replay_determinism(report: &FuzzReport) -> Result<(), VerifyError> {
    if report.passed {
        ...
        Err(VerifyError::DeterminismFailure(
            report.error_message.clone().unwrap_or_default(),
        ))
    }
}
...
pub fn run_replay_determinism_suite(seed: u64) -> Result<TestReport, VerifyError> {
    ...
    if report1.projection_hash == report2.projection_hash {
        Ok(TestReport {
            passed: true,
            tests_run: 1,
            failures: vec![],
        ...
        Ok(TestReport {
            ...
            failures: vec![TestFailure {
                test_name: "determinism".to_string(),
                message: format!(
                    "Hash mismatch: {} vs {}",
                    report1.projection_hash, report2.projection_hash
                ),
            }],
        })
    }
...
pub fn run_crash_recovery_scenario(db_path: &std::path::Path) -> Result<TestReport, VerifyError> {
    let _bootstrap = crate::store::bootstrap_store(db_path)
        .map_err(|e| VerifyError::TestHarness(e.to_string()))?;
    let check = crate::store::integrity_check(db_path);
    match check {
        Ok(_) => Ok(TestReport {
            passed: true,
            tests_run: 1,
        ...
        Err(e) => Ok(TestReport {
            ...
            tests_run: 1,
            failures: vec![TestFailure {
                test_name: "crash_recovery".to_string(),
                message: e.to_string(),
            }],
        }),
    }
...
pub fn assert_recovery_properties(report: &TestReport) -> Result<(), VerifyError> {
    if report.passed {
        ...
        let messages: Vec<String> = report.failures.iter().map(|f| f.message.clone()).collect();
        Err(VerifyError::TestFailure(messages.join("; ")))
    }
}
