...
replacements = [
    ("clear_clipboard()", "crate::ui::commands::CLIPBOARD.with(|s| *s.borrow_mut() = None)"),
...
if "thread_local! { pub static CLIPBOARD" not in content:
    with open(file, "w") as f:
        f.write("thread_local! { pub static CLIPBOARD: std::cell::RefCell<Option<crate::ui::commands::Clipboard>> = std::cell::RefCell::new(None); }\n" + content)

# 2. Pipeline.rs
file = "src/mutation/pipeline.rs"
replacements = [
    ("tags: Vec::new()", "tags: im::Vector::new()"),
    ("bend_points: Vec::new()", "bend_points: im::Vector::new()"),
    ("tags: vec![tag.clone()]", "tags: im::vector![tag.clone()]"),
    ("bend_points: vec![Point { x: OrderedFloat(100.0), y: OrderedFloat(i as f64 * 20.0) }]", "bend_points: im::vector![Point { x: OrderedFloat(100.0), y: OrderedFloat(i as f64 * 20.0) }]"),
...
if "pub enum ValidationPolicy" not in content:
    with open(file, "w") as f:
        f.write("pub enum ValidationPolicy { Strict, Permissive }\nimpl Default for ValidationPolicy { fn default() -> Self { Self::Strict } }\n" + content)

# 3. Canvas interaction reducer
replace_in_file("src/ui/canvas/interaction_reducer.rs", [("tags: Vec::new()", "tags: im::Vector::new()")])

# 4. Canvas perf
replace_in_file("src/ui/canvas/perf.rs", [
...
replace_in_file("src/ui/toolbar/persistence.rs", [("tags: Vec::new()", "tags: im::Vector::new()")])
