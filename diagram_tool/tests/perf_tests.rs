...
mod unit_tests {
    ...
    fn benchmark_result_passed_status() {
        ...
        let samples: Vec<FrameSample> = (0..10)
            ...
            .collect();
        let fps_report = FpsReport::from_samples(samples, 125.0);
        let result = BenchmarkResult::new(config.clone(), fps_report, 0);

        assert!(result.passed, "Should pass when FPS exceeds target");
        ...
        assert!(!result.passed, "Should fail when FPS is below target");
    }
