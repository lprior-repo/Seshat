...
(async () => {
  ...
  console.log("Creating edge using synthetic event dispatch (mimicking pointerdown)...");
  ...
  // Create an edge by dispatching custom pointerdown on the window explicitly to trigger the JS global handler
  await page.evaluate(({x, y}) => {
     const ev = new PointerEvent('pointerdown', { clientX: x, clientY: y, button: 0, bubbles: true, cancelable: true });
     window.dispatchEvent(ev);
     const ev2 = new PointerEvent('pointerup', { clientX: x, clientY: y, button: 0, bubbles: true, cancelable: true });
