...
// ============== SEL-001: Multi-type selection (shape+text) ==============
...
#[cfg(kani)]
#[kani::proof]
fn kanii_given_multi_type_selection_when_bounds_requested_then_all_types_included() {
    let mut doc = DiagramDocument::default();
    let shape_id = NodeId::new(String::from("shape_node"));
    let text_id = NodeId::new(String::from("text_node"));
...
#[cfg(kani)]
#[kani::proof]
fn kanii_given_nodes_at_negative_coords_when_selected_then_bounds_correct() {
    let mut doc = DiagramDocument::default();
    let neg_x = NodeId::new(String::from("neg_x"));
    let neg_y = NodeId::new(String::from("neg_y"));
    let neg_both = NodeId::new(String::from("neg_both"));
