...
mod test_events_module {
    ...
    async fn test_events_module_imports_async_store_error() -> Result<(), AsyncStoreError> {
        ...
        let valid_event = diagram_tool::store_async::envelope_to_valid_event(&envelope)?;
...
mod test_snapshot_module {
    ...
    async fn test_snapshot_works_with_async_store() -> Result<(), AsyncStoreError> {
        ...
        let valid_event1 = diagram_tool::store_async::envelope_to_valid_event(&envelope1)?;
...
mod test_append_event_async_full_roundtrip {
    ...
    async fn test_append_and_fetch_verifies_data_integrity() -> Result<(), AsyncStoreError> {
        ...
        let valid_event = diagram_tool::store_async::envelope_to_valid_event(&original_envelope)?;
    ...
    async fn test_revision_mismatch_error_is_typed() -> Result<(), AsyncStoreError> {
        ...
        let wrong_valid_event = diagram_tool::store_async::envelope_to_valid_event(&wrong_envelope)?;
        let mismatch_result = append_event_async(&pool, wrong_valid_event, Some(diagram_tool::store::types::Revision::new(5).unwrap())).await;
...
mod test_edge_cases {
    ...
    async fn test_duplicate_op_id_returns_error() -> Result<(), AsyncStoreError> {
        ...
        let valid_event1 = diagram_tool::store_async::envelope_to_valid_event(&envelope)?;
        let first_result = append_event_async(&pool, valid_event1, None).await?;
        assert_eq!(first_result.revision, 1);

        // Idempotent append not implemented - should return error on duplicate
        let valid_event2 = diagram_tool::store_async::envelope_to_valid_event(&envelope)?;
        let duplicate_result = append_event_async(&pool, valid_event2, None).await;
