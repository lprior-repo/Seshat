...
def fix():
    ...
    # Replace internal awaits in export.rs
    ...
    code = code.replace(
        "let mut conn2 = bootstrap2.pool;", "let conn2 = bootstrap2.pool;"
    )
    code = code.replace("handle.conn", "handle.pool")

    code = re.sub(
        r"store::bootstrap_store\((.*?)\)\.unwrap\(\)",
        r"store::bootstrap_store(\1).await.unwrap()",
        code,
    )
    code = re.sub(
        r"store::append_event\(&mut bootstrap\.pool, (.*?)\)\.unwrap\(\)",
        r"store::append_event(&bootstrap.pool, \1).await.unwrap()",
        code,
    )
    code = re.sub(
        r"store::append_event\(&mut conn, (.*?)\)\.unwrap\(\)",
        r"store::append_event(&conn, \1).await.unwrap()",
        code,
    )
    code = re.sub(
    ...
    # Some missed let result = export_diagram_json(...)
    code = re.sub(
