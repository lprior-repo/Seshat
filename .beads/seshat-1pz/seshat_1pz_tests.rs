...
// 
// Run with: cargo test --package diagram_tool --test seshat_1pz_tests
// 
...
use diagram_tool::store_async::{
    bootstrap_async_store, create_async_pool, read_store_pragmas_async, AsyncStorePragmas,
    AsyncStoreError,
};
...
/// Then: All PRAGMA values are correct (journal_mode, synchronous, wal_autocheckpoint, foreign_keys, busy_timeout)
#[tokio::test]
...
async fn given_data_in_wal_when_checkpoint_forced_then_data_persisted() {
    ...
    let (temp_dir, pool, _pragmas) = create_test_pool_with_pragmas()
        .await
        .expect("Failed to create test pool");
    ...
    let checkpoint_result: (i32, i32, i32) = sqlx::query_as("PRAGMA wal_checkpoint(TRUNCATE)")
        .fetch_one(&pool)
        .await
        .expect("Failed to checkpoint");

    // Then: checkpoint should succeed (busy=0 means success)
    // Result format: (busy, log, checkpointed)
    assert_eq!(checkpoint_result.0, 0, "Checkpoint should succeed");
}
...
// Note: Full async/sync parity tests require access to the sync store
// which may be in a different module. These tests verify the async store
// configuration matches the documented contract (synchronous=NORMAL).
...
/// The actual parity test would require initializing both stores.
#[tokio::test]
